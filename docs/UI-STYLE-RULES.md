...
Màu sắc của text **chỉ sử dụng** các class sau để duy trì tính đồng nhất và chuyên nghiệp. Cấm tuyệt đối việc sử dụng các dải màu text khác tự phát:
- `text-primary`: Dùng cho văn bản chính, văn bản có tính chất nhấn mạnh hoặc tiêu đề (nếu không dùng text-accent).
- `text-neutral-300`: Dùng cho văn bản phụ, ghi chú nhỏ, text có tính chất mô tả.
- `text-neutral-400`: Dùng cho văn bản phụ thứ cấp (nhấn mạnh ít hơn), placeholder phụ trợ.
