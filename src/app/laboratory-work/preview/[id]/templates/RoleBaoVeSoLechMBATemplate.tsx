...
export const RoleBaoVeSoLechMBATemplate = ({ job, device, categories, renderResults }: TemplateProps) => {
  let sectionIndex = 1;
              ...
              <table className="w-full text-[13pt] border-collapse info-table tech-info-table" style={{ tableLayout: "fixed" }}>
                <tbody>
                              ...
                              return (
                                <div className="space-y-0.5">
                                  {Array.from({ length: cols }).map((_, c) => {
                                    const colIndex = c + 1;
                                    const phaseName = parsed.data[`serial_1_${colIndex}`] || parsed.data[`serial_1-${colIndex}`] || "";

                                    // Collect all available tiers (Row 2, 3, etc.) for this column
                                    const tierValues = Array.from({ length: rows - 1 }).map((_, r) => {
                                      const rowIndex = r + 2;
                                      return parsed.data[`serial_${rowIndex}_${colIndex}`] || parsed.data[`serial_${rowIndex}-${colIndex}`] || "";
                                    }).filter(v => v !== "");

                                    return (
                                      <div key={c} className="flex flex-wrap gap-x-3 text-[13pt]">
                                        <span className="font-normal">{phaseName}</span>
                                        {tierValues.map((v, idx) => (
                                          <span key={idx}>{v}</span>
                                        ))}
                                      </div>
                                    );
                  ...
                  <tr>
                    ...
                    <td className="w-1/2 px-2 py-0" style={{ width: "50%" }}>
                      Điện áp định mức U<sub>R</sub> (kV) <span className="italic text-[11pt]">(Rated voltage)</span>: <span>{device?.nominalVoltage || ""}</span>
                    </td>
                  </tr>
                  <tr>
                    <td className="w-1/2 px-2 py-0" style={{ width: "50%" }}>
                      Dòng phóng danh định (kA) <span className="italic text-[11pt]">(Nominal discharge current)</span>: <span>{(device as any)?.nominalDischargeCurrent || ""}</span>
                    </td>
                    <td className="w-1/2 px-2 py-0" style={{ width: "50%" }}>
                      Điện áp vận hành liên tục U<sub>C</sub> (kV) <span className="italic text-[11pt]">(Continuous operating voltage)</span>: <span>{(device as any)?.continuousOperatingVoltage || ""}</span>
                    </td>
                  </tr>
                  <tr>
                    <td className="w-1/2 px-2 py-0" style={{ width: "50%" }}>
