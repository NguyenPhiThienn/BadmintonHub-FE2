...
export const MayBienDongDienTemplate = ({ job, device, categories, renderResults }: TemplateProps) => {
  ...
  return (
    <div className="flex flex-col gap-10 bg-white text-black print:gap-0">
      <div className="a4-page shadow-2xl print:shadow-none print:m-0 break-after-page" style={{ fontFamily: "'Tinos', 'Times New Roman', Times, serif" }}>
        <div className="p-1 flex-1 flex flex-col box-border" style={{ border: BORDER }}>
          <div className="flex-1 flex flex-col box-border">
            <div className="overflow-hidden" style={{ border: BORDER, borderBottom: 0 }}>
              ...
              <div className="p-3 text-center bg-white">
                <div className="text-[14pt] font-bold text-[#0070BF] uppercase leading-tight">
                  BIÊN BẢN THÍ NGHIỆM MÁY BIẾN DÒNG ĐIỆN
                </div>
            ...
            <div>
              <table className="w-full text-[13pt] border-collapse info-table tech-info-table" style={{ tableLayout: "fixed" }}>
                <tbody>
                  <tr>
                    <td colSpan={2} className="px-2 py-0">
                      Tên dự án/tên trạm <span className="italic text-[11pt]">(Project)</span>: <span>{job.projectName}</span>
                    </td>
                  </tr>
                  <tr>
                    <td colSpan={2} className="px-2 py-0">
                      Vị trí lắp đặt <span className="italic text-[11pt]">(Site)</span>: <span>{device?.site || device?.operatingName}</span>
                    </td>
                  </tr>
                  <tr>
                    <td className="w-1/2 px-2 py-0 align-top" style={{ width: "50%" }}>
                      Hãng sản xuất <span className="italic text-[11pt]">(Manufacturer)</span>: <span>{device?.manufacturer || ""}</span>
                    </td>
                    <td className="w-1/2 px-2 py-0 align-top" style={{ width: "50%" }}>
                      <div className="flex flex-col">
                        <div className="whitespace-nowrap mr-1">Số chế tạo <span className="italic text-[11pt]">(Serial Nº)</span>:</div>
                        <div className="flex-1">
                          {(() => {
                                      ...
                                      return (
                                        <tr key={r} style={{ border: 'none' }}>
                                          {Array.from({ length: maxCol }).map((_, cIdx) => {
                                            ...
                                            return <td key={c} className="p-0 pr-2 !border-none text-[13pt] font-normal leading-tight" style={{ border: 'none', padding: '0 4px 2px 0' }}>{val}</td>;
                                          })}
                                        </tr>
                                      );
                                    })}
                            ...
                            } catch { return device.serialJson; }
                          })()}
                        </div>
                      </div>
                    </td>
                  ...
                  <tr>
                    <td className="w-1/2 px-2 py-0" style={{ width: "50%" }}>
                      Năm sản xuất <span className="italic text-[11pt]">(Year of manufacture)</span>: <span>{device?.productionYear || ""}</span>
                    </td>
                    <td className="w-1/2 px-2 py-0" style={{ width: "50%" }}>
                      Kiểu máy <span className="italic text-[11pt]">(Type)</span>: <span>{device?.deviceModel || ""}</span>
                    </td>
                  </tr>
                  <tr>
                    <td className="w-1/2 px-2 py-0" style={{ width: "50%" }}>
                      Tỷ số biến <span className="italic text-[11pt]">(Ratio)</span>: <span>{device?.transformationRatio || ""}</span>
                    </td>
                    <td className="w-1/2 px-2 py-0" style={{ width: "50%" }}>
                      Cấp chính xác <span className="italic text-[11pt]">(Accuracy)</span>: <span>{device?.accuracyClass || ""}</span>
                    </td>
                  </tr>
                  <tr>
                    <td className="w-1/2 px-2 py-0" style={{ width: "50%" }}>
                  ...
                  <tr>
                    <td colSpan={2} className="px-2" style={{ borderBottom: 0 }}>
                      Điều kiện môi trường <span className="italic text-[11pt]">(Ambient condition)</span>: t <span className="italic text-[11pt]">(Temp.)</span> = <span className="">{job.temperature}°C</span>, Độ ẩm <span className="italic text-[11pt]">(Humidity)</span> = <span className="">{job.humidity}%</span>
                    </td>
                  </tr>
                </tbody>
              </table>
            ...
            {/* Items Summary Table */}
            <table className="w-full border-collapse text-[13pt]" style={{ borderLeft: BORDER, borderRight: BORDER, borderBottom: BORDER }}>
              <thead>
                ...
                <div className="leading-tight">
                  ...
                  <span className="font-normal text-[13pt]">
                    {(() => {
                      ...
                      return <>{vn} <span className="italic text-[11pt] font-normal">({en})</span></>;
                    })()}
                  </span>
                </div>
              </div>
              ...
              <div className="flex justify-between px-4 text-center pb-10">
                <div className="w-1/3 flex flex-col items-center">
                  ...
                  <div className="h-16 flex items-center justify-center">
                    {job.testerId?.digitalSignature && <img src={job.testerId.digitalSignature} className="h-16 w-auto object-contain" alt="Signature" />}
                  </div>
                  <div className="text-left font-normal pl-2 space-y-1 mt-2">
                    {job.testerName?.split(',').map((name: string, idx: number) => <div key={idx}>{idx + 1}. {name.trim()}</div>)}
                  </div>
                </div>
                ...
                <div className="w-1/3 flex flex-col items-center">
                  ...
                  <div className="h-16 flex items-center justify-center">
                    {job.approverId?.digitalSignature && <img src={job.approverId.digitalSignature} className="h-16 w-auto object-contain" alt="Signature" />}
                  </div>
                ...
                <div className="w-1/3 flex flex-col items-center">
                  ...
                  <div className="h-16 flex items-center justify-center">
                    {job.directorId?.digitalSignature && <img src={job.directorId.digitalSignature} className="h-16 w-auto object-contain" alt="Signature" />}
                  </div>
        ...
        {/* Web Preview Only: Page Footer (Puppeteer will use its own) */}
              ...
              <div className="space-y-1 text-[13pt]">
                {job.testingTools?.split(';').map((tool: any, i: number) => (
                  <div key={i} className="flex justify-start"><span className="w-full text-black">{tool.trim()}</span></div>
                    ...
                    <div className="italic text-neutral-400 font-normal">Chưa cập nhật thiết bị thí nghiệm</div>
                  )}
              </div>
            </div>
          </div>
