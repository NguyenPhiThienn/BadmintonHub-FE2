...
import { mdiAccountOutline, mdiCalendarMonthOutline, mdiLogout, mdiBadminton, mdiChevronLeft, mdiCrosshairsGps, mdiFire, mdiMagnify, mdiMapMarkerRadius, mdiRefresh, mdiTagOutline, mdiHeartOutline, mdiBellOutline, mdiCheckCircle, mdiCreditCardOutline, mdiClockOutline, mdiStorefrontOutline } from "@mdi/js";
...
const MapExplorerContent = () => {
  ...
  const [sortBy, setSortBy] = useState<string | null>(searchParams?.get("sortBy") || "favorites");
  ...
  const favoriteIds = favData?.data?.venues?.map((v: any) => v._id) || [];
  ...
  const rawVenues = venuesData?.data?.venues || [];
  const venues = sortBy === 'favorites' 
    ? rawVenues.filter((v: IVenue) => favoriteIds.includes(v._id))
    : rawVenues;
    
  const pagination = venuesData?.data?.pagination;
  const hasMore = pagination ? rawVenues.length < pagination.total : false;
  ...
  const handleSortChange = (newSortBy: string) => {
    ...
    if (newSortBy === 'favorites') {
      if (!user) {
        toast.warn("Vui lòng đăng nhập để xem danh sách yêu thích");
        return;
      }
    }
  ...
  const filterButtons = [
    { id: 'favorites', label: 'Yêu thích', icon: mdiHeartOutline },
  ...
  return (
    <div className="flex h-screen w-full bg-darkBackgroundV1 overflow-hidden">
      ...
      <div className="relative flex-1 h-full">
        ...
        {/* Top Right Desktop Avatar */}
        <div className="absolute top-4 right-4 z-10 hidden md:flex items-center">
          {user ? (
            <div className="flex items-center gap-1.5 bg-darkCardV1/80 border border-darkBorderV1 p-1.5 rounded-full shadow-lg backdrop-blur-md">
                {/* Avatar Dropdown */}
                <DropdownMenu modal={false}>
                    <DropdownMenuTrigger asChild>
                        <Avatar className="h-10 w-10 border border-darkBorderV1 hover:border-accent transition-all rounded-full cursor-pointer">
                            <AvatarImage
                    ...
                    <DropdownMenuContent className="w-72 bg-darkCardV1 border border-darkBorderV1 shadow-2xl rounded-xl p-2" align="end" forceMount>
                        ...
                        <DropdownMenuItem asChild className="cursor-pointer rounded-lg p-3 text-neutral-300 focus:text-white focus:bg-white/10 hover:bg-white/10 transition-colors">
                            <Link href="/" className="flex items-center gap-3">
                                <Icon path={mdiBadminton} size={0.9} />
                                <span className="font-medium text-[15px]">Trang chủ</span>
                            </Link>
                        </DropdownMenuItem>
                        <DropdownMenuItem asChild className="cursor-pointer rounded-lg p-3 text-neutral-300 focus:text-white focus:bg-white/10 hover:bg-white/10 transition-colors">
            ...
            <Button variant="accent" className="shadow-lg shadow-black/20" onClick={() => setAuthMode("login")}>
                Đăng nhập
            </Button>
          )}
        </div>
...
const MapExplorer = () => {
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-darkBackgroundV1 flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          <div className="w-12 h-12 border-4 border-accent/30 border-t-accent rounded-full animate-spin" />
          <p className="text-neutral-400 animate-pulse text-sm">Đang tải bản đồ...</p>
        </div>
      </div>
    }>
      <MapExplorerContent />
