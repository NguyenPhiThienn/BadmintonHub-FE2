        ...
        if (isOpen) {
            ...
            if (initialData && mode === "edit") {
                form.reset({
                    ...
                    openTime: initialData.openTime || "00:00",
                    closeTime: initialData.closeTime || "24:00",
    ...
    return (
        <>
            ...
            <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
                <DialogContent size="medium" onInteractOutside={(e) => {
                    ...
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                            <div className="space-y-4 md:space-y-4 max-h-[70vh] overflow-y-auto pr-1 custom-scrollbar p-3 md:p-4">
                                            ...
                                            <FormItem>
                                                ...
                                                <FormControl>
                                                    <div className="space-y-3">
                                                        ...
                                                        <div id="venue-map-preview" className="w-full h-[220px] rounded-lg overflow-hidden border border-darkBorderV1 bg-darkBackgroundV1/50 hidden" />
                                                    </div>
                                                </FormControl>
                                                <FormMessage />
                                            </FormItem>
                                ...
                                {/* SECTION: BẢNG GIÁ THEO KHUNG GIỜ - chỉ hiển thị khi edit (đã có venueId) */}
                                {mode === "edit" && initialData?._id && (
                                    <div className="mt-4">
                                        <PricingSection
                                            venueId={initialData._id}
                                            defaultPricePerHour={initialData.pricePerHour ?? 0}
                                        />
                                    </div>
                                )}
                                {mode === "create" && (
                                    <div className="mt-4 flex items-center gap-2 text-xs text-neutral-500 bg-darkBackgroundV1/30 border border-dashed border-darkBorderV1 rounded-lg px-3 py-3">
                                        <span>💡 Bảng giá theo khung giờ có thể cấu hình sau khi tạo cơ sở xong.</span>
                                    </div>
                                )}
                                ...
                                <div className="flex items-center gap-3 md:gap-4 mt-4">
                                    <h3 className="text-accent font-semibold whitespace-nowrap flex items-center gap-1.5">
                                        ...
                                        Danh sách sân con ({courtFields.length})
                                    </h3>
                                    <div className="flex-1 border-b border-dashed border-accent mr-1" />
                                    <Button
                                        type="button"
                                ...
                                <div className="flex flex-col gap-3 mb-4">
                                    {courtFields.map((field, index) => (
                                        <div key={field.id} className="relative p-3 rounded-lg border border-darkBorderV1 bg-darkBackgroundV1/30 flex flex-col sm:flex-row sm:items-end gap-3 transition-colors hover:border-accent/40">
                                            <div className="flex-1">
                                                <FormField
                                                    ...
                                                    render={({ field }) => (
                                                        <FormItem>
                                                            {index === 0 && <FormLabel className="text-xs text-neutral-400">Tên sân</FormLabel>}
                                                            <FormControl>
                                                                <Input placeholder="Nhập tên sân (vd: Sân 1)" {...field} className="bg-darkCardV1" />
                                                            </FormControl>
                                                            <FormMessage />
                                                        </FormItem>
                                                    )}
                                            ...
                                            <div className="flex-1">
                                                <FormField
                                                    ...
                                                    render={({ field }) => {
                                                        const predefinedTypes = ["Sàn gỗ", "Thảm PVC", "Sàn nhựa", "Sàn xi măng"];
                                                        const isCustom = customTypeModes[index] || (field.value && !predefinedTypes.includes(field.value));

                                                        return (
                                                            <FormItem>
                                                                {index === 0 && <FormLabel className="text-xs text-neutral-400">Loại sàn</FormLabel>}
                                                                <FormControl>
                                                                    {isCustom ? (
                                                                        <div className="relative">
                                                                            <Input placeholder="Nhập loại sàn..." {...field} className="bg-darkCardV1 pr-8" autoFocus />
                                                                            <Button
                                                                                type="button"
                                                                                variant="ghost"
                                                                                className="absolute right-0 top-0 h-full px-2 text-neutral-400 hover:text-white"
                                                                                onClick={() => {
                                                                                    setCustomTypeModes(prev => ({ ...prev, [index]: false }));
                                                                                    field.onChange("Sàn gỗ");
                                                                                }}
                                                                            >
                                                                        ...
                                                                        <Select
                                                                            ...
                                                                            onValueChange={(val) => {
                                                                                if (val === "CUSTOM") {
                                                                                    setCustomTypeModes(prev => ({ ...prev, [index]: true }));
                                                                                    field.onChange("");
                                                                                } else {
                                                                                    field.onChange(val);
                                                                                }
                                                                        ...
                                                                        >
                                                                            <SelectTrigger className="bg-darkCardV1">
                                            ...
                                            <Button
                                                type="button"
                                                variant="ghost"
                                                className="text-red-400 hover:text-red-300 hover:bg-red-500/10 sm:mb-[2px] w-full sm:w-auto"
                                                onClick={() => removeCourt(index)}
                                                disabled={courtFields.length === 1}
                                            >
                                                <Icon path={mdiTrashCanOutline} size={0.9} />
                                                <span className="sm:hidden ml-2">Xóa sân</span>
                                            </Button>
                                        </div>
                                    ))}
                                </div>
