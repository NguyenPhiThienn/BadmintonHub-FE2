...
export const LoginForm = ({ onSwitchRegister, onSwitchForgot, isDialog, onSuccess }: LoginFormProps) => {
  ...
  const { loginUser: setUserContext, fetchUserProfile, forceLogout } = useUser();
  ...
  const onSubmit = async (data: any) => {
    try {
      ...
      // Xử lý lỗi tài khoản bị khóa từ backend
      const errorData = error?.response?.data;
      if (error?.status === 403 || errorData?.message?.includes('bị khóa')) {
        const blockedReason = errorData?.blockedReason || 'Không có lý do được cung cấp';
        forceLogout(blockedReason);
      } else {
