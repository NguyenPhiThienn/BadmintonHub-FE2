...
function DesktopLayout({ children }: { children: React.ReactNode }) {
  ...
  const { data: pendingVenuesRes } = usePendingVenues({ page: 1, limit: 1 });

  const profile = profileResponse?.data;
  const permissions = (profile as any)?.permissions || [];
  const role = (profile as any)?.role;
  const pendingVenuesCount = pendingVenuesRes?.data?.pagination?.total || 0;
  const dashboardMenuItems: MenuItem[] = getDashboardMenuItems(permissions, role, pendingVenuesCount);
...
function TabletLayout({ children }: { children: React.ReactNode }) {
  ...
  const role = (profile as any)?.role;
  const pendingVenuesCount = pendingVenuesRes?.data?.pagination?.total || 0;
  const menuItems = getDashboardMenuItems(permissions, role, pendingVenuesCount);
