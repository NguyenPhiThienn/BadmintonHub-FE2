...
import { useCreateTestJob, useTestCategoryTree, useTestJobs } from "@/hooks/useTesting";
...
export const AddJobDialog = ({ isOpen, onClose, device }: AddJobDialogProps) => {
    const {
        formData,
        activeTab,
        setFormData,
    ...
    const { data: categoryTreeResponse, isLoading: isLoadingTree } = useTestCategoryTree(
        //@ts-ignore
        typeof device?.deviceTypeId === "string" ? device.deviceTypeId : (device?.deviceTypeId as any)?._id || ""
    );
    ...
    const { mutate: createJob, isPending } = useCreateTestJob();

    // Reset form when device changes or dialog opens
    useEffect(() => {
        if (isOpen && device && device._id !== formData.deviceId) {
            resetFormData(device._id);
        }
    }, [isOpen, device, formData.deviceId, resetFormData, setFormData]);
    ...
    const handleResultChange = (categoryId: string, field: string, value: any) => {
        const currentTestResults = useTestingJobFormStore.getState().formData.testResults;
        setTestResults({
            ...currentTestResults,
            [categoryId]: {
                ...(currentTestResults[categoryId] || { evaluation: "", isSkipped: false, value: "" }),
                [field]: value
            }
        });
    };
    ...
    const handleSubmit = () => {
        ...
        const { site, ...restFormData } = formData;
        const payload = {
            ...restFormData,
            deviceId: device._id,
            ...
            <DialogContent size="medium" className="max-w-4xl">
                <DialogHeader>
                    <DialogTitle className="flex items-center gap-2 text-accent">
                        ...
                        <span>Thêm mới công việc thí nghiệm</span>
                    </DialogTitle>
                </DialogHeader>
                    ...
                    <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                        ...
                        <TabsContent value="info">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div className="md:col-span-2 flex items-center gap-3 md:gap-4 w-full mt-2">
...
// --- Sub-components for Test Results ---
...
const TestCategorySection = ({ category, results, onChange }: TestCategorySectionProps) => {
    ...
    const handleEvalChange = (v: string) => {
        const cascadeUpdate = (cat: ITestCategory, newEval: string) => {
            onChange(cat._id, "evaluation", newEval);
            if (cat.children && cat.children.length > 0) {
                cat.children.forEach(child => cascadeUpdate(child, newEval));
            }
        };
        cascadeUpdate(category, v);
    };

    const handleSkipChange = (checked: boolean) => {
        const cascadeUpdate = (cat: ITestCategory, skip: boolean) => {
            onChange(cat._id, "isSkipped", skip);
            if (cat.children && cat.children.length > 0) {
                cat.children.forEach(child => cascadeUpdate(child, skip));
            }
        };
        cascadeUpdate(category, checked);
    };
        ...
        return (
            <Card className="mb-4 border-accent/30 overflow-hidden bg-darkCardV1">
                ...
                <div className="bg-darkBackgroundV1 border-b border-accent/20 p-3 py-2 flex flex-wrap items-center justify-between gap-3">
                    ...
                    <h6 className="font-semibold text-neutral-300 mb-0 text-sm flex-1 text-wrap">{category.name}</h6>
    ...
    return (
        <div className={cn("mb-4 pb-4 border-b border-darkBorderV1 last:border-0 last:mb-0 last:pb-0 relative")}>
            <div className="flex items-center justify-between gap-3 mb-2">
                ...
                <Label className="font-semibold text-neutral-300 mb-0 flex-1 text-wrap normal-case text-sm">{category.name}</Label>
                <div className="flex items-center gap-2">
...
const DynamicTableControl = ({ config, value, onChange }: { config: any, value: any, onChange: (v: any) => void }) => {
    // config structure: [{"Name":"STT","Unit":"","IsChart":false}, ...]
                ...
                <table className="w-full text-sm text-center border-collapse">
                    <thead>
                        <tr className="bg-darkBackgroundV1/50">
                            ...
                            <th className="p-2 border border-darkBorderV1 w-10"></th>
                        </tr>
                    </thead>
                    <tbody>
                        {rows.map((row, rIndex) => (
