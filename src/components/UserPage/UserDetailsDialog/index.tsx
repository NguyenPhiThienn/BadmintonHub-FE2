            ...
            <DialogContent size="medium">
                ...
                <div className="space-y-4 md:space-y-4 max-h-[70vh] overflow-y-auto custom-scrollbar p-3 md:p-4">
                    {!isEditing ? (
                                        ...
                                        <Card className="flex-1 p-0 overflow-hidden border border-darkBorderV1 bg-transparent">
                                            <Table>
                                                <TableBody>
                                                    ...
                                                    {(userData as any)?.status === 'BLOCKED' && (userData as any)?.blockedReason && (
                                                        <TableRow>
                                                            <TableCell className="text-neutral-300 font-semibold w-[160px]">
                                                                <div className="flex items-center gap-2">
                                                                    <Icon path={mdiAlertCircleOutline} size={0.6} className="flex-shrink-0" />
                                                                    <span className="text-nowrap">Lý do khóa</span>
                                                                </div>
                                                            </TableCell>
                                                            <TableCell colSpan={3}>
                                                                <Badge variant="red" className="text-left">
                                                                    {(userData as any).blockedReason}
                                                                </Badge>
                                                            </TableCell>
                                                        </TableRow>
                                                    )}
                        ...
                        <div className="space-y-6 md:space-y-6 pb-4">
                            {/* Account Status Card */}
                            <div className="p-4 border border-darkBorderV1 rounded-xl bg-darkCardV1 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                                <div>
                                    <h4 className="font-semibold text-white flex items-center gap-2 mb-1">
                                        <Icon path={mdiCheckCircleOutline} size={0.7} className="text-accent" />
                                        Trạng thái tài khoản
                                    </h4>
                                    <p className="text-sm text-neutral-400">
                                        {(userData as any)?.status === 'BLOCKED' 
                                            ? 'Tài khoản này đang bị khóa và không thể đăng nhập hoặc đặt sân.' 
                                            : 'Tài khoản đang hoạt động bình thường.'}
                                    </p>
                                    {(userData as any)?.status === 'BLOCKED' && (userData as any)?.blockedReason && (
                                        <p className="text-xs text-red-400 mt-2 bg-red-500/10 p-2 rounded inline-block border border-red-500/20">
                                            Lý do: {(userData as any).blockedReason}
                                        </p>
                                    )}
                                </div>
                                <Button 
                                    variant={(userData as any)?.status === 'BLOCKED' ? "default" : "destructive"}
                                    className={(userData as any)?.status === 'BLOCKED' ? "bg-green-600 hover:bg-green-700" : ""}
                                    onClick={() => onBlock?.(userData)}
                                >
                                    <Icon path={(userData as any)?.status === 'BLOCKED' ? mdiLockOpenVariantOutline : mdiLockOutline} size={0.8} />
                                    {(userData as any)?.status === 'BLOCKED' ? 'Mở khóa ngay' : 'Khóa tài khoản'}
                                </Button>
                            </div>

                            <div>
                                <div className="flex items-center gap-3 md:gap-4 mb-4">
                                    <h3 className="text-accent font-semibold whitespace-nowrap">Cập nhật thông tin cơ bản</h3>
                                    <div className="flex-1 border-b border-dashed border-accent mr-1" />
                                </div>
                ...
                <DialogFooter>
                    ...
                    {isEditing ? (
                        <Button
                            onClick={handleSave}
                            disabled={isUpdating || isBlocking}
                        >
                            <Icon
                                path={isUpdating || isBlocking ? mdiLoading : mdiContentSaveOutline}
                                size={0.8}
                                className={isUpdating || isBlocking ? "animate-spin" : ""}
                            />
                            {isUpdating || isBlocking ? "Đang xử lý..." : "Lưu thay đổi"}
                        </Button>
                    ) : (
                        <Button
